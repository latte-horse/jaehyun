...
for keyword_source in keywordsource_dic.keys():
    ...
    for i, keywords in enumerate(keywords_list):
        ...
        for j, news in enumerate(naver_list):
            df.loc[len(df.index)] = [keyword_source, keywords, i, 'NAVER', j, news['title'], news['link']]
            
        print("{} 검색어 {} DAUM 뉴스 수집 시작... ".format(keyword_source, i+1), flush=True)
        daum_list =  daumFuncs.getNewsList(keywords, 30)
        for j, news in enumerate(daum_list):
            df.loc[len(df.index)] = [keyword_source, keywords, i, 'DAUM', j, news['title'], news['link']]
        # #print("검색어 {} 뉴스 Google 수집 시작... ".format(keyword_site, i+1), flush=True)
        # google_list = googleFuncs.getNewsList(keywords, 3)
print("모든 뉴스 수집 완료")
