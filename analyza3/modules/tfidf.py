...
#------------------------------------------------------------------------------
# IssueWhatShow 용도에 맞게 각각의 키워드에 대한 문서들이 TF 값을 정규화 시킴
# 각각의 키워드에 대한 문서들의 중요도를 평준화 시키는 효과를 얻는다.
#------------------------------------------------------------------------------
def normalize_tf(mergScores):
    scoreList = list(map(
            lambda mergeScore : mergeScore['TF_score'], mergScores))
    min_ = min(scoreList)
    max_ = max(scoreList)
    for mergScore in mergScores:
        mergScore['TF_score'] = (mergScore['TF_score'] - min_) / (max_ - min_)
...
def get_sigwords_by_tf(path, searchword, group):
    ...
    # 합산된 TF 값을 정규화
    mergScores = normalize_tf(mergScores)
